...
# Instructions
This lab asks you to solve the problems below and answer the questions in an rmarkdown docu-
ment. You should first create a new rmarkdown document (see https://faculty.washington.
edu/otoomet/info201-book/r-markdown.html#r-markdown-rstudio-creating) and write com-
plete answers (with complete sentences). Several of the questions ask you to inject the calcu-
lated numbers into the sentences as inline code chunks (see https://faculty.washington.edu/
otoomet/info201-book/r-markdown.html#inline-code), but some of the answers are expected
to be a code chunk with both code and results visible, and your comments underneath.
Have fun!
...
```{r}
# income-height data
incomeheight <- read_delim("data/income-height.csv.bz2")
```
...
`r str_flatten_comma(names(incomeheight), last = ", and ")`. 
...
Missing values: 
*income variable*: `r sum(is.na(incomeheight$income))`
*height variable*: `r sum(is.na(incomeheight$height))`


```{r}
sum(is.na(incomeheight$income))
sum(is.na(incomeheight$height))
```
